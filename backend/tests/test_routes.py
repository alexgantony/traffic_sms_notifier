...
# Delete route
def test_delete_route_success(client, test_route, auth_header):
    del_response = client.delete(
        f"/routes/{test_route['route_id']}", headers=auth_header
    )
    assert del_response.status_code == 204

    response = client.get(f"/routes/{test_route['route_id']}", headers=auth_header)
